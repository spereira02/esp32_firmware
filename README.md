...
[!IMPORTANT]
You must log out and log back in (or reboot) for group changes to take effect. Verify with the groups command.
...
Troubleshooting
Baud Rate: If the session fails to establish, specify the baud rate explicitly: --baudrate 921600.

Port Access: Ensure no other serial monitors (Arduino IDE, etc.) are accessing the port.

Architecture: If deploying on Raspberry Pi 5, ensure the uros_ws was built locally on the Pi to match the ARM64 architecture.
